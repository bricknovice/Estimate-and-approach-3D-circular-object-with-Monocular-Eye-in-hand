...
A fast and effective YaedEllipse::Ellipse detector for embedded vision applications
...
class CEllipseDetectorYaed
{
	...
	int ACC_R_SIZE;			// cv::Size of accumulator R = rho = atan(K)
...
public:
	...
	void DetectAfterPreProcessing(std::vector<YaedEllipse::Ellipse>& ellipses, cv::Mat1b& E, cv::Mat1f& PHI);

	//Detect the ellipses in the gray image
	void Detect(cv::Mat1b& gray, std::vector<YaedEllipse::Ellipse>& ellipses);
	
	//Draw the first iTopN ellipses on output
	void DrawDetectedEllipses(cv::Mat3b& output, std::vector<YaedEllipse::Ellipse>& ellipses, int iTopN=0, int thickness=2);
...
private:
	...
	void ClusterEllipses(std::vector<YaedEllipse::Ellipse>& ellipses);
	...
	void FindEllipses	(	cv::Point2f& center,
							...
							EllipseData& data_ij,
							EllipseData& data_ik,
							std::vector<YaedEllipse::Ellipse>& ellipses
						);
